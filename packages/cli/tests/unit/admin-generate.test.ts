...
describe("adminGenerateCommand", () => {
  ...
  it("writes the generated composition file with one factory per admin entry", async () => {
    ...
    expect(content).toContain(`import { createFooAdminExtension } from "@voyant-travel/foo-react/admin"`)
  ...
  it("honors a package.json voyant.adminEntry override", async () => {
    writeFileSync(join(tmp, "voyant.config.ts"), `export default { modules: ["@voyant-travel/baz"] }\n`)
    ...
    expect(code).toBe(0)
    const content = readFileSync(join(tmp, "src", "admin.extensions.generated.ts"), "utf8")
    expect(content).toContain(`import { createBazAdminExtension } from "@voyant-travel/baz-admin/entry"`)
...
describe("adminGenerateCommand --routes — redirects + nested children", () => {
  ...
  beforeEach(() => {
    ...
    writeFileSync(join(tmp, "voyant.config.ts"), `export default { modules: ["@voyant-travel/foo"] }\n`)
